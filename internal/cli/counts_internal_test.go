...
func TestPrintScanSummaryLine(t *testing.T) {
	...
	require.NoError(t, printScanSummary(cmd, &App{errStyle: NewStyler(false)}, 254, 891, 0, 72*time.Second))

	assert.Equal(t, "scanned: 254 chats, matched: 891 files, took 1m12s\n", errBuf.String())
}

func TestPrintScanSummaryWithDuplicates(t *testing.T) {
	t.Parallel()

	cmd, errBuf := newErrCmd()

	require.NoError(t, printScanSummary(cmd, &App{errStyle: NewStyler(false)}, 3, 7, 2, 2*time.Second))

	assert.Equal(t, "scanned: 3 chats, matched: 7 files, duplicates: 2, took 2s\n", errBuf.String())
}
...
func TestPrintScanSummaryColoredAndSilentSuppressed(t *testing.T) {
	...
	require.NoError(t, printScanSummary(cmd, &App{errStyle: NewStyler(true)}, 3, 7, 0, 2*time.Second))
	...
	require.NoError(t, printScanSummary(silent, &App{}, 3, 7, 0, time.Second))
